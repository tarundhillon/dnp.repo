define('timeline',['moment','dnphelper','semui'],
...
function makeTimeLine(data, conf){
	...
	var minDate = d3.min(data, function(d) {return dateFormat.parse(d['Date']);});
	var maxDate = d3.max(data, function(d) {return dateFormat.parse(d['Date']);});
...
var points = svg.selectAll('.points')
	...
	.attr('cx',function(d){ return d.ex;})
	.attr('cy',-conf.lineHeight/2)
	.attr('r',outR)
	.attr('stroke',conf.pointColor)
	.attr('fill',conf.lineColor)
	...
	.attr('fill',conf.pointColor)
	.attr('cnt',function(d,i){
		//console.log('outR point draw '+i+' : '+d.objList[0].Date);
		return d.objList.length;
	})
	.on('click',conf.pointOnClick);
	// .on('click',function(d){
	...
	.style("stroke", conf.pointColor)
	.style("stroke-dasharray", ("3, 3"))
	.attr("x1", function(d){return d.ex;})
	.attr("y1", -conf.lineHeight/2)
	.attr("x2", function(d){return d.ex;})
	.attr("y2",function(d,i){return getLevelY(d,i);});

	textBox = points.append('g');

	textBox.append('rect') // text box
	.style("fill", conf.textBoxColor)
	.style("stroke", conf.pointColor)
	...
	.attr("x", function(d){return d.ex - conf.textBoxWidth/2;})
	.attr("y", function(d,i){return getLevelY(d,i);})
	.attr('class',function(d,i){
		if(i%2 === 0) return 'topRect';
		else return 'bottomRect';
	})
	.attr("width", conf.textBoxWidth)
	.attr("height",conf.textBoxHeight)
	.on('click',conf.pointOnClick);
...
var getLevelY = function(d,i){
	...
	if(even) return svgDim.height/2 - ((conf.textBoxHeight+conf.textBoxPad) *level) - conf.textBoxHeight - conf.textBoxPad;
	else return -svgDim.height/2 + ((conf.textBoxHeight+conf.textBoxPad) *level) + conf.textBoxPad;
};

function offsetDate(d,offset){
	switch(conf.dateUnit){
		case 'year': return d3.time.year.offset(d,offset);
		case 'month': return d3.time.month.offset(d,offset);
		case 'week': return d3.time.week.offset(d,offset);
		case 'day': return d3.time.day.offset(d,offset);
	}
}
...
function autoDimention(setting){
	...
	var svgDim = { height:setting.maxSvgHeight,
					width:domWidth,
					fontSize: '0.8rem',
