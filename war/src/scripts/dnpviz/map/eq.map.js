require(['/src/scripts/js/dnp.config.js'],function(){
	require(['eqMap'],function(eqMap){ loadMap();});
});

var moment;
define('eqMap',['jquery', 'd3','dnphelper','moment','d3Topo'],
	function( $, d3, dhelp, moment,d3Topo){
		setupPage();
		d3Geo = require(['d3Geo']);
		//d3Topo = require(['d3Topo']);
		this.moment = moment;
...
var eqConf = {
	...
	mapUrl: '/src/scripts/dnpviz/map/world.svg',
	//dataUrl: 'http://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/all_day.geojson',
	dataUrl: 'http://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/all_month.geojson',
	//dataUrl: 'http://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/significant_month.geojson',
	topoJsonUrl: '/src/scripts/dnpviz/map/world-110m2.json',
	pointColor: '#DB2828',
	pointPert: 0.75,
	minHeight: 500,
	maxHeight: 500,
...
function mapQuakes(data){
	...
	var quakes = allQuakes.filter(function(obj){
		var objTime = moment(obj.properties.time), curTime = moment();
		switch(eqConf.dataFilter){
			case 'today': return curTime.diff(objTime,'days') === 0;
			case 'pastWeek': return curTime.diff(objTime,'days') <= 7;
			case 'pastMonth': return curTime.diff(objTime,'days') <= 30;
		}
	});
...
function showPointDetails(d,i,minMag){
	...
	{"mag":5.1,"place":"145km S of Kushiro, Japan","time":1457138003350,
	"updated":1457166966355,"tz":600,
	...
	"felt":null,"cdi":null,"mmi":null,"alert":null,"status":"reviewed",
	"tsunami":0,"sig":400,"net":"us","code":"10004uv6","ids":",us10004uv6,",
	"sources":",us,","types":",geoserve,nearby-cities,origin,phase-data,tectonic-summary,",
	"nst":null,"dmin":0.813,"rms":0.65,"gap":73,"magType":"mb",
	"type":"earthquake","title":"M 5.1 - 145km S of Kushiro, Japan"},
	"geometry":{"type":"Point","coordinates":[144.1459,41.6745,24.2]},"id":"us10004uv6"}
	*/
...
function setDimention(eqConf){
	...
	var svgDim = { height:eqConf.maxHeight,
					width:domWidth,
					fontSize: '0.8rem',
