define('projcontrol', ['gantt', 'moment', 'dnpHelper', 'semanticui'],
    function(gantt, moment, _help, semui) {
        console.log('project control dependencies loaded');

        var isRefreshAttached = false;
        ...
        function makeGanttForWeek(week) {
            ...
            if (week != 'task') {
                ganttData.data = ganttData.fulldata.filter(function(item) {
                    return item.week == ganttData.week;
                });
            ...
            if (ganttSetting.refreshOnResize && !isRefreshAttached) {
                $(window).on('resize', function() {
                    console.log('resize detected');
                    makeGanttForWeek(week);
                });
                isRefreshAttached = true;
            }
        ...
        function makeGanttDataTable(ganttData, ganttSetting) {
            ...
            ganttData.data.forEach(function(obj) {
                ...
                html += "<td><div class='ui empty " + _help.getRAGColor(obj.rag) + " label'></div></td>";
                html += "<td>" + taskAlert.pertHTML + " " + _help.intPad(obj.pert, 2) + "%</div></td>";
        ...
        function calTaskAlerts(obj) {
            ...
            var prevObjs = ganttData.fulldata.filter(function(item) {
                return item.id === obj.id && item.week === prevWeek;
            });
        ...
        function makeTimelineHeader(ganttData, ganttSetting) {
            ...
            ganttData.weekList.forEach(function(week) {
                ...
                html += "<a href=\"javascript: _proj.makeGanttForWeek('" + week + "')\" class='ui " + active + " label'>" + week + "</a>";
            });
            html += "</div>";
        ...
        var ganttTaskOnClick = function(task) {
            ...
            var taskData = [],
                obj = {};
            ganttData.weekList.forEach(function(week) {
                weekTask = ganttData.fulldata.filter(function(d) {
                    return d.week == week && d.id == task.id;
                });
        ...
        var ganttCatOnClick = function(category) {
            ...
            var taskData = [],
                obj = {};
            ganttData.weekList.forEach(function(week) {
                weekTasks = ganttData.fulldata.filter(function(d) {
                    return d.week == week && d.type == category;
                });
        ...
        return {
            makeGanttAndTableView: makeGanttAndTableView,
            makeGanttForWeek: makeGanttForWeek,
            calTaskAlerts: calTaskAlerts,
            getPrevWeek: getPrevWeek,
            prepareGanttData: prepareGanttData,
            makeTimelineHeader: makeTimelineHeader,
            tableTaskOnClick: tableTaskOnClick,
            ganttTaskOnClick: ganttTaskOnClick,
            ganttCatOnClick: ganttCatOnClick,
            ganttEmptyObj: ganttEmptyObj
